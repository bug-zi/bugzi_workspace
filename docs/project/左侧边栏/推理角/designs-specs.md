...
**generateSoups(preference)**（v1.4 重写：两阶段管线，出题 temperature 0.9 / 审题 0.2，jsonMode）：

- **阶段一 · 出题（prompt v2）**：资深出题人人设 + 画像摘要；注入近 200 碗「汤名+题材」避免清单与**近 9 碗 trick_note 摘要清单**（新汤核心思路必须明显不同；批内 3 碗互不相似——软约束，**不设类型枚举**，巧思不受分类束缚）。**质量标准各配自检问句**：①常识可解（专业知识可作佐证、不得作解题钥匙——「普通人不查资料能否逼近汤底」）②对手不降智（破绽须源于固有盲区，「对手再谨慎一点还会漏吗」）③线索自然隐藏（≥3 个可盘问事实织入叙事，至多一半显性异常、**至少一条隐性线索**，禁报菜名式罗列）④难度三档定义（easy 单层反转线索较显眼 / medium 双层反转或有伪装需排除误导 / hard 多层反转或强误导关键线索全隐性）⑤汤底逐一回收 ⑥本格自洽 ⑦可判定。输出 schema 增 `trick_note`（15~40 字，只述思路不复述案情）；解析失败自动重试一次，不足 3 碗抛错**不落半截数据**。
- **阶段二 · 逐碗串行审题（reviewSoup，260907 二调：硬伤才打回）**：串行 for 循环逐碗审（**不并行**——中转通道有账号并发上限，3 趟并行会触发 429 退避共振），每碗材料 = 三件套 + 自评 + trick_note + 近期清单 + 同批他碗摘要；审题人**先只读汤面**记录玩家视角一手证据（第一直觉 / 想盘问的问题 / 一眼异常），再读汤底——立场「**默认放行，拿不准算过**」，只打回硬伤：①常识门槛（关键一环需查资料的专业知识）②逻辑硬伤（因果断裂 / 悬空元素 / 巧合堆砌 / 不可判定）③同构（对近期清单与同批他碗）④极端报菜名（异常全一眼可见且无任何平常线索）。线索自然与难度复核为提示性意见**不打回**，难度仍按三档定义独立重评（**落库以审题人评定为准**）；指定偏好时重评不符由代码硬校验计入打回一次。审题调用/解析失败经 `reviewSoupSafe` **重试一次**，两次均失败才弃碗（通道故障与汤质量无关）。
- **打回与落库**：不合格碗携问题清单**重出一次**（不给原碗内容防锚定；注入保留碗 trick_note 互避；补位难度档 = 偏好档（指定模式）或审题人重评档（random 模式，维持批内错开）），复审**无硬伤即入碗**——难度偏好仍不符时按审题人重评档如实落库（260907 二调，不再弃碗）；复审仍有硬伤**弃碗不弃批**——合格碗照常入库返回实际 `{generated, inserted}`，3 碗全灭才抛错（文案「本批汤未通过审题或生成通道不稳定，未落库，请稍后重试」）。INSERT 含 trick_note（status=fresh）。
