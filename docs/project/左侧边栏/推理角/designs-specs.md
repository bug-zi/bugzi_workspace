...
**DB v22（v1.6 思维墙题型换血）**：精选题库追加 8 道思维游戏题——`electron/db/wallBankSeed.ts` 新增导出 `WALL_BANK_SEED_V22`（4 道经开发者例题校准：侦探 hard「雨夜失窃的怀表」/ 情境 medium「1208 房」/ 文字 medium「祖父的信」/ 生活 hard「额头上的红漆」；4 道待人工验证：侦探 easy「碎玻璃」/ 情境 easy「周日晚上的电话」/ 文字 medium「老先生的对联」/ 生活 medium「消失的坚果」），source 统一「AI 起草」，tag 即题型中文名；迁移只插入不动旧行，`WALL_BANK_SEED` 保持 v13 原样防全新用户在 v13 重复插入。**题库无 hints 字段**（三级提示仅为每日一题/练习场机制）。注：v1.3 规划的 wall_pool（顺延 v16）未实施——代码无此表，v22 直接顺延实际版本号（v17 海龟汤对局字段、v18 海龟汤计时、v19 书架、v20 信息源、v21 账本均为他模块当日占用）。
...
- [ ]  **v1.6 题型换血**：重启触发 v22 迁移后题库新增 8 题（共 18）；每日一题/练习场出的均为新四类（侦探/情境/文字/生活），无数学理论题；练习场四类下拉各出至少一题；旧类型每日一题记录显示中文名正常；判答宽松等价按题型语境生效（接近真相的表述判对）
