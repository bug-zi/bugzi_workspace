...
新功能开发区待完成项实施（`左侧边栏/学习模块/万象库/2026-09-12-知识问答标签页-design.md` + 同日 plan）：万象库第三 tab「问答」——偏科普知识问题，AI 强制 MCP 联网搜索后回答。单轮存档（追问走右栏「万象·问答」频道不绑定记录）、安静模式（过程不推频道）、允许重复提问、不联动百科词条。DB v37 qa_records；runQaAnswer 复用辩真「关键词→检索→综合」骨架；删除入回收站万象库块（新 source qa，恢复/3 天清沿用）。typecheck 双配置通过；运行时冒烟待开发者验证。
```
