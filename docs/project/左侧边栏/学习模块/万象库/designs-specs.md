...
> 本文档由 AI 基于 `docs/project/左侧边栏/学习模块/万象库/design.md` 与《总需求文档.md》第 1、5、6、8 条生成，是开发的直接依据。依赖：样式/designs-specs.md 的 MdDialog 与 AI 边栏、回收站/designs-specs.md 的接入约定。
...
- 卡片内划词交互（仅 MdDialog 打开的万象卡片内启用，MdDialog 需支持按模块开关此能力）：
  ...
  - **取消高光**（260921 第49轮增量）：选区整体落在既有 `<mark>` 高光内时，气泡改出「取消高光」（按整段标记文本取消，局部选中同段整体取消）+「问 AI」——md 解除 `==标记==`（replaceAll）+ 删 wiki_highlights 对应记录（IPC `wiki:removeHighlight`），MdDialog key 含 mdVersion 重挂保证高光/取消即时渲染。高光内再划高光的嵌套脏写路径随之堵死。笔记本删除仍按原约定仅移除记录、不动卡片标记。
