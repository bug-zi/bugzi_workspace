...
**Tech Stack:** Electron 44 + React 19 + TypeScript + node:sqlite + marked。设计文档：`docs/project/左侧边栏/学习模块/学习库/2026-09-11-学习库-design.md`（实施前先通读）。
...
**Files:**
- Modify: `docs/project/左侧边栏/学习模块/学习库/2026-09-11-学习库-design.md`
...
```markdown
- [X]  左侧边栏上面新添加一个模块（放在第一个的位置）——关于我的专业知识的学习库。具体如何设计需要好好商议（260911 已实施，`左侧边栏/学习模块/学习库/2026-09-11-学习库-design.md` + 同日 plan；DB v33 四表 + AI 建树/卡片/展开三函数 + learnStock 每日队列定档与预生成泵 + LearnModule 三 tab（今日学习/知识树/高光笔记）+ MdDialog studyBar + 右栏「学习·问答」频道 + 回收站九块；typecheck 双配置通过，运行时冒烟待开发者验证）
```
...
- 依据 `左侧边栏/学习模块/学习库/2026-09-11-学习库-design.md` 与同日 plan 完成：DB v33（learn_domains / learn_nodes / learn_daily / learn_highlights + 出厂 8 领域 seed）、AI 三函数（建树/知识卡/主题展开）、learnStock（每日队列定档纯 SQL + 预生成泵）、learn:* IPC 全量、LearnModule 三 tab、MdDialog studyBar、右栏「学习·问答」频道、回收站九块、启动/午夜触发接线。
