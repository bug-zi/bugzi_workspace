...
运行 `release/bugzi-workspace-Setup-1.0.0.exe`（上一版安装包，若已装 1.0.0 可跳过）。
...
启动已装的 1.0.0 App：
1. 启动 ~5s 后 toast「发现新版本 v1.0.1」，个人档「版本」小节进 available 态（新版本行 + 下载更新按钮）。
2. 点「下载更新」→ 进度条推进到 100% → 出「重启并安装」。
3. 点「重启并安装」→ 静默安装、App 自动重启 → 个人档显示 `v1.0.1`，用户数据（各模块数据/背景图/头像）完好。
