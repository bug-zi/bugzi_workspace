...
**Architecture:** 文件系统为真相源（`userData/bg/light/`、`userData/bg/dark/` 两目录、唯一文件名、列表=扫目录），当前使用仍走现有 settings 键 `bg_bg-light`/`bg_bg-dark`——ThemeProvider/bzres 协议零改动；主进程新增 4 个 `image:bg*` IPC + 一次性幂等迁移；渲染层新 BgLibraryDialog 弹窗 + ProfileModule 替换原两行按钮。
