...
- md 目录：`md/turtle/`（对局记录 `{gameId}.md`）、`md/wall/`（每日题详情 `{YYYY-MM-DD}.md`）、`md/wall/bank/`（题库详情 `{id}.md`——v1.7 起策展题与练习场生成题共用）。
...
- `answering`：题面卡片（题型 + 难度 badge，**v1.7 题面 MdView 渲染**）+ 多行作答输入 + 「提交作答」+「要提示（已用 N/3）」——提示直接取库存下一级（不调 LLM），用尽后按钮文案「提示已用完」禁用。
...
**WallPanel · 练习场（v1.1 增补，design v2 备选「练习场」提前落地；v1.7 生成题入库）：**
...
**WallPanel · 题库（v1.2 双层题源第二层，打卡墙与练习场之间；260909 由「精选题库」改名并收入练习场生成题）：**

- 人工策展的存量难题随 DB v13/v22 种子入库（现行 8 题），**v1.7 起练习场 AI 生成题同表自动入库**（source 区分：种子「AI 起草」/ 练习场「AI 练习场生成（日期）」），**AI 只判答不出题**（题库面板自身无出题入口，练习场出的题在此沉淀）；目录 `md/wall/bank/{id}.md` 局终一次写入（作答/看解答/练习场判答回写共用 `writeBankMd`）。
- 列表行：状态图标（未做 help / 已破 task_alt / 未破 close）+ 标题 + 标签 badge + 难度 badge + 出处（悬停 title）；头部计数「已破 N / 总数」；点行打开当前题卡。
- 当前题卡：**题面 MdView 渲染**（v1.7）+ 多行作答输入（结论 + 论证，结论对即算对）+「提交作答」（判答 → 结果内联呈现 + 列表状态刷新）+「看解答」（**二次确认**，终态记 failed 直接揭示标准答案与论证）。终态后再点行 = 结果回看 +「查看完整档案」开 MdDialog。
...
- **硬性质量标准**（逐条自检）：答案唯一可判定；题面自包含、关键线索全在题面；不可被模板套解、非脑筋急转弯；至少一条「看似可行的错误解释」；hints 三级递进（方向→方法→关键一步，不直接给答案）；reasoning 标准论证全文（200~500 字，闭合无跳步）。输出 `{title, puzzle, answer, reasoning, hints[3]}`——**v1.7 增 `title`**（题目短标题，10 字内点出题材不剧透；练习场生成题入题库用，每日一题忽略；LLM 未给时字段可缺，入题库侧兜底取题面首行截 16 字）。
...
wall:practiceNew(pref)                   → { id, puzzle, typeZh, diffZh, hintsTotal }（v1.1 练习场：
                                           ...
                                           v1.2 加 typePref 题型可选；v1.3 优先从 wall_pool
                                           按难度/题型筛选取题——DELETE 池行后入 practiceBank
                                           （取走视为消耗，不恢复），无匹配兜底现场生成；
                                           **v1.7 生成即 INSERT wall_bank todo 态**（source
                                           「AI 练习场生成（日期）」，title 取出题 title 字段
                                           兜底题面首行），bankId 存入 practiceBank entry）
wall:practiceAnswer(id, myAnswer)        → { correct, standardAnswer, explanation }（一题一命判答即终局；
                                           id 失效（重启）抛 PRACTICE_GONE；**v1.7 回写题库
                                           终态 + writeBankMd 详情 md**（该行已终态则跳过））
wall:practiceHint(id)                    → { level, text } | null（内存直取不调 LLM；用尽/失效 null）

wall:bankList()                          → [{ id, title, tag, diffZh, difficulty, source, status,
                                           mdPath }]（题库列表（v1.7 改名），不泄答案与论证）
...
- [ ]  **v1.7 练习题入库**：练习场出题后题库列表即现新行（todo，标题为出题短标题，出处含「AI 练习场生成」）；判答后该行落 solved/failed 且「查看完整档案」可开详情 md；弃做换题后行留 todo 可在题库侧补答；同题在题库侧先答后练习场提交不双写
- [ ]  **v1.7 题面渲染**：每日一题 / 题库 / 练习场三处题面的加粗、列表等 md 语法渲染生效，不再显示 `**` 等原文记号
